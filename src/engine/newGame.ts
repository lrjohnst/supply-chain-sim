import type { GameState, CityNode, HarborNode, MapLink, Corporation, Firm } from "../types";
...
export function newGame(playerName: string): GameState {
  ...
  // Seed AI with a store in its starting city
  ...
  const nodes: CityNode[] = [
    {
      ...
      type: "city",
      population: 800_000,
      firmSlots: GameConfig.firmSlots.large_city,
      energyCostMultiplier: 1.0,
      hasHarborAccess: false,
    ...
    },
    {
      ...
      type: "city",
      population: 500_000,
      firmSlots: GameConfig.firmSlots.medium_city,
      energyCostMultiplier: 0.9,
      hasHarborAccess: true,
    ...
    // Towns
    {
      ...
      type: "town",
      population: 80_000,
      firmSlots: GameConfig.firmSlots.town,
    ...
    // Harbor node (represented as a geographic point)
    {
      id: "harbor_city",
      name: "Port Verano",
      type: "harbor",
      population: 120_000,
      firmSlots: 0, // no firms can be built at harbor
      energyCostMultiplier: 1.0,
      hasHarborAccess: true,
...
function buildMapLinks(nodes: Record<string, CityNode>): Record<string, MapLink> {
  ...
  for (const [from, to] of connections) {
    ...
    links[id] = {
      ...
      fromNodeId: from,
      toNodeId: to,
      baseCost: GameConfig.map.defaultLinkCost,
...
function seedAI(state: GameState, aiCorpId: string): void {
  ...
  state.firms[firmId] = {
    id: firmId,
    corporationId: aiCorpId,
    cityNodeId: cityId,
    type: "store",
    name: `${aiCorp.name} Store`,
